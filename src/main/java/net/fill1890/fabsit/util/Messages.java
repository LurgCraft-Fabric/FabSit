...
public class Messages {
    ...
    public static TranslatableText getPoseStopMessage(ServerPlayerEntity player, Pose pose) {
        if(ConfigManager.loadedPlayers.contains(player.networkHandler.connection.getAddress())) {
            ...
            return new TranslatableText(ConfigManager.LANG.get(ACTION + "stop_" + pose).formatted("the sneak key"));
        }
    }

    // get either a server or client translated string based on whether the player has the mod
    private static TranslatableText getChatMessageByKey(ServerPlayerEntity player, String key_base) {
        if(ConfigManager.loadedPlayers.contains(player.networkHandler.connection.getAddress())) {
            return new TranslatableText(CHAT + key_base);
        } else {
            return new TranslatableText(ConfigManager.LANG.get(CHAT + key_base));
        }
    }

    // trying to pose in midair
    public static TranslatableText getMidairError(ServerPlayerEntity player, Pose pose) {
    ...
    public static TranslatableText getSpectatorError(ServerPlayerEntity player, Pose pose) {
        return getChatMessageByKey(player, switch(pose) {
    ...
    public static TranslatableText getStateError(ServerPlayerEntity player, Pose pose) {
        return getChatMessageByKey(player, switch (pose) {
    ...
    // pose disabled
    public static TranslatableText poseDisabledError(ServerPlayerEntity player, Pose pose) {
        return getChatMessageByKey(player, switch (pose) {
    ...
    public static TranslatableText configLoadSuccess(ServerPlayerEntity player) {
        return getChatMessageByKey(player, "reload_success");
    }

    public static TranslatableText configLoadError(ServerPlayerEntity player) {
        return getChatMessageByKey(player, "reload_error");
    }

    public static TranslatableText blockOccupiedError(ServerPlayerEntity player, Pose pose) {
        return getChatMessageByKey(player, switch(pose) {
    ...
    public static void sendByException(ServerPlayerEntity player, Pose pose, PoseException e) {
        if(e instanceof MidairException) {
            player.sendMessage((Text) getMidairError(player, pose), false);
        } else if(e instanceof SpectatorException) {
            player.sendMessage((Text) getSpectatorError(player, pose), false);
        } else if(e instanceof StateException) {
            player.sendMessage((Text) getStateError(player, pose), false);
        } else if(e instanceof PoseDisabled) {
            player.sendMessage((Text) poseDisabledError(player, pose), false);
        } else if(e instanceof BlockOccupied) {
            player.sendMessage((Text) blockOccupiedError(player, pose), false);
        }
    }
}
